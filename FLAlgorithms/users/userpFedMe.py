...
class UserpFedMe(User):
    def __init__(self, device, numeric_id, train_data, test_data, model, batch_size, learning_rate, beta, L_k,
                 local_epochs, K, personal_learning_rate):
        ...
        self.optimizer = pFedMeOptimizer(
            self.model.parameters(), lr=self.personal_learning_rate, L_k=self.L_k)
        ...
        for epoch in range(1, self.local_epochs + 1):  # local update
            ...
            # K = 30 # K is number of personalized steps
                for i in range(self.K):
                    ...
                    loss.backward()
                    self.persionalized_model_bar, _ = self.optimizer.step(
                        self.local_model)

                # update local weight after finding aproximate theta
                for new_param, localweight in zip(self.persionalized_model_bar, self.local_model):
                    localweight.data = localweight.data - self.L_k * \
                        self.learning_rate * \
